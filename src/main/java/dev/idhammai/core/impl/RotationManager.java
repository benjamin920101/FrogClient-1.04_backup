...
import dev.idhammai.api.events.impl.DoAttackEvent;
import dev.idhammai.api.events.impl.FireworkShooterRotationEvent;
import dev.idhammai.api.events.impl.InteractBlockEvent;
import dev.idhammai.api.events.impl.InteractItemEvent;
import dev.idhammai.api.events.impl.JumpEvent;
import dev.idhammai.api.events.impl.KeyboardInputEvent;
...
import dev.idhammai.api.events.impl.TravelEvent;
...
import dev.idhammai.mod.modules.settings.enums.SnapBack;
...
implements Wrapper {
    ...
    public void update(SendMovementPacketsEvent event) {
        if (AntiCheat.INSTANCE.movementSync() && !BaritoneUtil.isActive()) {
            ...
            Frog.EVENT_BUS.post(updateRotateEvent);
    ...
    public void onLastRotation(UpdateRotateEvent event) {
        ...
        Frog.EVENT_BUS.post(rotationEvent);
    ...
    public void setLastYaw(float lastYaw) {
        ...
        if (AntiCheat.INSTANCE.forceSync.getValue() && Frog.SERVER.playerNull.passedS(0.15)) {
