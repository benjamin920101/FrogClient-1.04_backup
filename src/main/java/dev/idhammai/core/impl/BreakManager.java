...
import dev.idhammai.api.events.impl.BlockBreakingProgressEvent;
...
implements Wrapper {
    ...
    public void onTick(ClientTickEvent event) {
        ...
        if (AntiCheat.INSTANCE.detectDouble.getValue()) {
            Iterator<Object> iterator = ((ConcurrentHashMap.KeySetView)Frog.BREAK.doubleMap.keySet()).iterator();
            while (iterator.hasNext()) {
                int i = (Integer)iterator.next();
                BreakData breakData = Frog.BREAK.doubleMap.get(i);
    ...
    public void onPacket(PacketEvent.Receive event) {
        ...
        if (packet instanceof BlockBreakingProgressS2CPacket) {
            ...
            Frog.EVENT_BUS.post(BlockBreakingProgressEvent.get(packet2.getPos(), packet2.getEntityId(), packet2.getProgress()));
