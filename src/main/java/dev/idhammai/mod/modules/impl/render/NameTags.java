...
extends Module {
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        for (PlayerEntity playerEntity : Frog.THREAD.getPlayers()) {
    ...
    private String getNametagInfo(PlayerEntity player) {
        ...
        if (this.totemsConfig.getValue() && (totems = Frog.POP.getPop(player)) > 0) {
    ...
    private int getNametagColor(PlayerEntity player) {
        if (this.friendConfig.booleanValue && player.getDisplayName() != null && Frog.FRIEND.isFriend(player)) {
