...
extends Module {
    ...
    private Color getFillColor(PlayerEntity player) {
        return Frog.FRIEND.isFriend(player) ? this.fillFriend.getValue() : this.fill.getValue();
    }

    private Color getBoxColor(PlayerEntity player) {
        return Frog.FRIEND.isFriend(player) ? this.boxFriend.getValue() : this.box.getValue();
    }

    @Override
    public void onRender3D(MatrixStack matrixStack) {
        for (BreakManager.BreakData breakData : Frog.BREAK.breakMap.values()) {
        ...
        if (this.second.getValue()) {
            Iterator<BreakManager.BreakData> iterator = ((ConcurrentHashMap.KeySetView)Frog.BREAK.doubleMap.keySet()).iterator();
            while (iterator.hasNext()) {
                int i = (Integer)((Object)iterator.next());
                BreakManager.BreakData breakData = Frog.BREAK.doubleMap.get(i);
                if (breakData == null || breakData.getEntity() == null || BreakESP.mc.world.isAir(breakData.pos)) {
                    Frog.BREAK.doubleMap.remove(i);
                    continue;
                }
                BreakManager.BreakData singleBreakData = Frog.BREAK.breakMap.get(i);
