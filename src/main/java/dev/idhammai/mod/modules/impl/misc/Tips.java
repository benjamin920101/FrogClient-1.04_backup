...
import dev.idhammai.api.events.impl.DeathEvent;
import dev.idhammai.api.events.impl.EntitySpawnEvent;
import dev.idhammai.api.events.impl.PacketEvent;
import dev.idhammai.api.events.impl.RemoveEntityEvent;
import dev.idhammai.api.events.impl.TotemEvent;
...
extends Module {
    ...
    public void onAddEntity(EntitySpawnEvent event) {
        ...
        boolean isFriend = Frog.FRIEND.isFriend(playerName);
    ...
    public void onPlayerDeath(DeathEvent event) {
        ...
        if (this.popCounter.getValue()) {
            if (Frog.POP.popContainer.containsKey(player.getName().getString())) {
                int l_Count = Frog.POP.popContainer.get(player.getName().getString());
                if (l_Count == 1) {
    ...
    public void onTotem(TotemEvent event) {
        if (this.popCounter.getValue()) {
            ...
            int l_Count = 1;
            if (Frog.POP.popContainer.containsKey(player.getName().getString())) {
                l_Count = Frog.POP.popContainer.get(player.getName().getString());
            }
            if (l_Count == 1) {
                if (player.equals((Object)Tips.mc.player)) {
