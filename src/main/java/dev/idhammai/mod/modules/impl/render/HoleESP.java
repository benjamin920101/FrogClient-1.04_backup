...
extends Module {
    ...
    Type isHole(BlockPos pos) {
        if (HoleESP.mc.world.isAir(pos) && (!this.airYCheck.getValue() || pos.getY() == HoleESP.mc.player.getBlockY() - 1 || pos.getY() == HoleESP.mc.player.getBlockY()) && Frog.HOLE.isHard(pos.up())) {
            return Type.Air;
        }
        int blockProgress = 0;
        boolean bedRock = true;
        for (Direction i : Direction.values()) {
            if (i == Direction.UP || i == Direction.DOWN || !Frog.HOLE.isHard(pos.offset(i))) continue;
