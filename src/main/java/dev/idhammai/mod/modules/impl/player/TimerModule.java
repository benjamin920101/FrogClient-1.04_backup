    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        ...
        if (MovementUtil.isMoving() && !EntityUtil.isInsideBlock()) {
            ...
            if (this.timer2.passed(this.lastMs)) {
                ...
                double timer = (double)Frog.TIMER.getDefault() + (1.0 - this.end.ease(this.ease.getValue())) * (double)(this.shiftTimer.getValueFloat() - 1.0f) * ((double)this.lastMs / this.accumulate.getValue());
                Frog.TIMER.set((float)Math.max((double)Frog.TIMER.getDefault(), timer));
            } else {
                Frog.TIMER.set(this.shiftTimer.getValueFloat());
            }
        } else {
            if (this.moving) {
                Frog.TIMER.reset();
