...
extends Module {
                ...
                for (double z : offset) {
                    for (Direction i : Direction.values()) {
                        for (int d = 0; d < 3; ++d) {
                            ...
                            if (!this.crystalHere(blockerPos2) || this.placePos.contains(blockerPos2) || Frog.HOLE.isHard(aroundPos)) continue;
        ...
        if (this.burrow.getValue()) {
            for (double x : offset) {
                for (double z : offset) {
                    ...
                    if (this.isBedrock(surroundPos) || !Frog.BREAK.isMining(surroundPos)) continue;
                    ...
                    for (int i = 0; i < n; ++i) {
                        ...
                        if (this.detectMining.getValue() && Frog.BREAK.isMining(defensePos)) continue;
            ...
            for (double x : offset) {
                for (double z : offset) {
                    for (Direction i : Direction.values()) {
                        ...
                        for (Direction direction : Direction.values()) {
                            BlockPos blockPos;
                            if (direction == Direction.UP || direction == Direction.DOWN || !AutoCrystal.INSTANCE.canPlaceCrystal(blockPos = surroundPos.offset(direction), true, true) || this.detectMining.getValue() && Frog.BREAK.isMining(blockPos)) continue;
                            if (this.breakCrystal.getValue()) {
    ...
    public BlockPos getHelper(BlockPos pos) {
        for (Direction i : Direction.values()) {
            if (i == Direction.UP || this.detectMining.getValue() && Frog.BREAK.isMining(pos.offset(i)) || !BlockUtil.isStrictDirection(pos.offset(i), i.getOpposite()) || !BlockUtil.canPlace(pos.offset(i), 6.0)) continue;
            return pos.offset(i);
        }
        return null;
    }
