...
import dev.idhammai.api.utils.math.PredictUtil;
...
extends Module {
    ...
    private void trapTarget(PlayerEntity target) {
        if (this.onlyHole.getValue() && !Frog.HOLE.isHole(EntityUtil.getEntityPos((Entity)target))) {
            return;
        }
    ...
    private void doTrap(PlayerEntity player, BlockPos pos) {
        ...
        if (this.antiStep.getValue() && (Frog.BREAK.isMining(pos.up(headOffset)) || !this.onlyBreak.getValue())) {
        ...
        if (this.chestUp.getValue()) {
            for (Direction i : Direction.values()) {
                ...
                if (this.onlyBreaking.getValue() && !Frog.BREAK.isMining(pos.up(headOffset))) continue;
    ...
    public BlockPos getHelper(BlockPos pos) {
        ...
        for (Direction i : Direction.values()) {
            if (this.checkMine.getValue() && Frog.BREAK.isMining(pos.offset(i)) || !BlockUtil.isStrictDirection(pos.offset(i), i.getOpposite()) || !BlockUtil.canPlace(pos.offset(i), this.placeRange.getValue(), this.breakCrystal.getValue())) continue;
            return pos.offset(i);
        }
        return null;
    }
    ...
    public BlockPos getHelper(BlockPos pos, Direction ignore) {
        ...
        for (Direction i : Direction.values()) {
            if (i == ignore || this.checkMine.getValue() && Frog.BREAK.isMining(pos.offset(i)) || !BlockUtil.isStrictDirection(pos.offset(i), i.getOpposite()) || !BlockUtil.canPlace(pos.offset(i), this.placeRange.getValue(), this.breakCrystal.getValue())) continue;
            return pos.offset(i);
        }
        return null;
    }
    ...
    private boolean checkEntity(BlockPos pos) {
        ...
        for (Entity entity : Frog.THREAD.getPlayers()) {
