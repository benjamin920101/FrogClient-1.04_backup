...
extends Module {
    ...
    public ElytraFly() {
        ...
        INSTANCE = this;
        Frog.EVENT_BUS.subscribe(new FireWorkTweak());
    }
    ...
    public void off() {
        ...
        if (ElytraFly.mc.player.getMainHandStack().getItem() == Items.FIREWORK_ROCKET) {
            if (this.packetInteract.getValue()) {
                ElytraFly.sendSequencedPacket(id -> new PlayerInteractItemC2SPacket(Hand.MAIN_HAND, id, Frog.ROTATION.getLastYaw(), Frog.ROTATION.getLastPitch()));
            } else {
